...
def inference(model, motion_vq, audio_path, device, save_folder, sr, pose_fps,):
    ...
    with torch.no_grad():
        ...
        print("latents shapes:",
              "face", None if face_latent is None else tuple(face_latent.shape),
              "upper", None if upper_latent is None else tuple(upper_latent.shape),
              "hands", None if hands_latent is None else tuple(hands_latent.shape),
              "lower", None if lower_latent is None else tuple(lower_latent.shape))
        print("indices shapes:",
              "face", None if face_index is None else tuple(face_index.shape),
              "upper", None if upper_index is None else tuple(upper_index.shape),
              "hands", None if hands_index is None else tuple(hands_index.shape),
              "lower", None if lower_index is None else tuple(lower_index.shape))
    ...
    beat_format_save(os.path.join(save_folder, f"{base_name}_output.npz"),
